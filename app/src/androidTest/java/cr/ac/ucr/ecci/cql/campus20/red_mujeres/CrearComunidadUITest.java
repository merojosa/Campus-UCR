...
public class CrearComunidadUITest {
    ...
    public static final String ACTIVITY_TITLE = "Crear una nueva comunidad";
...
//    }
    ...
    public static final String TEST_COMMUNITY_NAME = "Comunidad_Test";
    ...
    @Test
    public void test_Title_Create_Community()     //Test para verificar que el título de la actividad es el correcto
    {
        Intent intent = new Intent();

        intentsTestRule.launchActivity(intent);
        onView(withId(R.id.text_Activity_Title)).check(matches(withText(ACTIVITY_TITLE)));
        onView(withId(R.id.button_Create_Community_Form)).check(matches(withText("Crear comunidad")));
    }

    @Test
    public void test_Complete_Interface()     //Test para verificar que la interfaz está completa
    {
        Intent intent = new Intent();

        intentsTestRule.launchActivity(intent);
        onView(withId(R.id.button_Create_Community_Form)).check(matches(withText("Crear comunidad")));
        onView(withId(R.id.text_Create_Community_Description)).check(matches(withText("Descripción")));
        onView(withId(R.id.text_Community_Name_Create_Form)).check(matches(withText("Nombre")));
    }

    @Test
    public void test_popUp()     //Test para verificar si se despliega el pop up de confirmación al pulsar el botón
    {
        Intent intent = new Intent();

        intentsTestRule.launchActivity(intent);
        onView(withId(R.id.editText_Create_Community_Name)).perform(typeText(TEST_COMMUNITY_NAME), closeSoftKeyboard());
        onView(withId(R.id.button_Create_Community_Form)).perform(click());
        onView(withText("¿Desea crear la nueva comunidad " + TEST_COMMUNITY_NAME +"?")).check(matches(isDisplayed()));
    }
